...
3. **Data-at-Rest Encryption**: Offline attachment blobs and local caches are encrypted at rest using `age` (X25519 / ChaCha20-Poly1305 streaming cipher) and AES-256-GCM column encryption (`PayloadCipher`).
4. **Daemon Security Enclave**: Plaintext secrets and private keys reside strictly inside `nunciod` heap memory protected by `ZeroizeOnDrop` memory wrappers. Local IPC streams validate socket peer identity (`SO_PEERCRED` UID checks on POSIX, Windows Named Pipe SID DACLs, and constant-time 256-bit bearer token verification).
5. **MCP Capability Safety & HITL Prompts**: Model Context Protocol (`nuncio-mcp`) tools operate under `Agent-Restricted` RBAC policies. Destructive actions (such as sending emails) trigger Human-in-the-Loop (HITL) prompt interception in presentation UIs before execution.
