        ...
        match name {
            "nuncio_mail_list" => {
                if !self.policy.is_data_type_allowed(DataType::Mail)
                    || !self.policy.permissions.read_mail
                {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' lacks 'read_mail' permission",
                ...
                if !self.policy.is_folder_allowed(folder_id) {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' is restricted from folder '{}'",
                        self.policy.agent_id, folder_id
                    ));
                }
                let messages = self
                    .db
            ...
            "nuncio_mail_send" => {
                if !self.policy.is_data_type_allowed(DataType::Mail)
                    || !self.policy.permissions.send_mail
                {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' lacks 'send_mail' permission",
                        self.policy.agent_id
                    ));
                }
                let account_id = args
                ...
                if !self.policy.is_account_allowed(account_id) {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' is restricted from account '{}'",
                        self.policy.agent_id, account_id
                    ));
                }
                let recipient = args
                    .get("recipient")
            ...
            "nuncio_cal_list_events" => {
                if !self.policy.is_data_type_allowed(DataType::Calendar)
                    || !self.policy.permissions.read_calendar
                {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' lacks 'read_calendar' permission",
                        self.policy.agent_id
                    ));
                }
            ...
            "nuncio_cal_create_event" => {
                if !self.policy.is_data_type_allowed(DataType::Calendar)
                    || !self.policy.permissions.write_calendar
                {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' lacks 'write_calendar' permission",
                        self.policy.agent_id
                    ));
                }
                let account_id = args
            ...
            "nuncio_account_list" => {
                if !self.policy.is_data_type_allowed(DataType::Mail) {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' lacks access to account list",
                        self.policy.agent_id
                    ));
                }
            ...
            "nuncio_filter_list" => {
                if !self.policy.is_data_type_allowed(DataType::FilterRules)
                    || !self.policy.permissions.manage_filters
                {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' lacks 'manage_filters' permission",
                        self.policy.agent_id
                    ));
                }
                let rules = self
            ...
            "nuncio_filter_create" => {
                ...
                let rule = nuncio_filter::NsqlParser::parse_rule(name, priority, sql)
                    .map_err(|e| e.to_string())?;
            ...
            "nuncio_filter_edit" => {
                ...
                let mut updated = nuncio_filter::NsqlParser::parse_rule(name, priority, sql)
                    .map_err(|e| e.to_string())?;
                updated.id = id.to_string();
                let opts = nuncio_filter::ValidationOptions::default();
                nuncio_filter::NsqlValidator::validate(&updated, &opts)
                    .map_err(|e| e.to_string())?;
                self.db
                    .save_filter_rule(&updated)
                    .await
                    .map_err(|e| e.to_string())?;
            ...
            "nuncio_filter_test" => {
                let sql = args
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or("missing sql")?;
                let rule = nuncio_filter::NsqlParser::parse_rule("Test Rule", 0, sql)
                    .map_err(|e| e.to_string())?;
                let engine =
                    nuncio_filter::FilterEngine::new(vec![rule]).map_err(|e| e.to_string())?;
            ...
            "nuncio_audit_list" => {
                ...
                let records = self
                    .db
                    .list_worm_audit_records(limit, offset)
                    .await
                    .map_err(|e| e.to_string())?;
            ...
            "nuncio_export_data" => {
                ...
                {
                    return Err(format!(
                        "403 Forbidden: Agent '{}' lacks 'read_mail' permission for data export",
                        self.policy.agent_id
                    ));
                }
                let output_str = args
                ...
                let format: nuncio_core::ExportFormat =
                    format_str.parse().map_err(|e: String| e)?;
                let messages = self
                    .db
                    .list_messages(folder_id, limit)
                    .await
                ...
                let summary = self
                    .db
                    .export_messages_to_file(&messages, format, std::path::Path::new(output_str))
                    .await
                    .map_err(|e| e.to_string())?;
