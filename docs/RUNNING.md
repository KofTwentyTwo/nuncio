...
Every numeric cell in the **accounts table** renders an em dash (`—`),
never `0`, when the poll cycle that would have supplied it failed -- a `0`
there is always a genuine reading, not a guess. This is backed by
`GetHealth`, which fails the whole RPC (rather than a per-field default)
