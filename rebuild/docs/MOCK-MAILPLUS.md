...
Host listeners bind 127.0.0.1 with ephemeral ports. Startup requires a local Docker Unix socket; every container operation is pinned to that validated socket, and cleanup retains it with the unique project identity. A dedicated Docker bridge supports these mappings; Docker29.7.2 did not publish ports for the attempted internal-network composition. The bridge itself is not an egress firewall. Automated requests use only the local endpoints; Mailpit relaying is not configured; reverse DNS and automatic GitHub version checks are disabled. Earlier Task12 gates preceded the version-check setting and do not prove that background update requests were absent. Explicit CI host/container egress denial remains tracked in Tasks03/15.
