...
Task 13 is in progress. Encrypted backup creation, inspection, and restore into a new profile now work through the engine, authenticated Maintenance API, and CLI. The actual daemon/CLI recovery test preserves a queued send, request identity, and a real PDF attachment; independent Google mock observations show no submission after restore/reconnect/restart, followed by exactly one submission after an explicit duplicate-risk resend decision. Projection repair is implemented across the same layers; the full schema20 repair gate passes, including independent Google and Synology system/subprocess tests. Schemas 1–21 have migration and historical backup-restore fixtures; every next-migration ledger failure rolls back without changing original encrypted bytes. All42 migration commit boundaries also pass actual-daemon SIGKILL/restart checks. Historical send attempts/receipts/decisions across schemas10–21 survive migration/restore with pending work held. Other write-kind histories, restored-operation reconciliation, crash cleanup and remaining resource checks are still required. Current commands, failures and evidence are in [SESSION-STATE.md](SESSION-STATE.md) and [VERIFICATION.md](VERIFICATION.md).
...
- Safe explicit reconciliation for every restored pending operation kind, with independent remote send/copy/notification evidence.
- Process-crash recovery for abandoned staging directories and new keystore entries, activation/parent races, disk-space and error injection coverage.
- Historical payloads for other write kinds and disconnect retention/same-identity reconnection checks.
- Broader adversarial stream and output errors, current production artifacts and complete offline release verification. Both-provider send recovery has offline subprocess evidence; this does not cover every operation kind.

The schema21 local production executables are verified at `target/production/release/{nunciod,nuncio-cli}` by the reconciliation-mutation gate; hashes are in `test-results/task13-reconciliation-mutations/artifacts.json`. Package and full release verification remain outstanding. No installation, live-provider acceptance or remote CI execution is implied.
