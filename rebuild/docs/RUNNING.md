...
The worker shares account sequencing and the global two-account limit with synchronization, bounds MIME preparation with one shared composition slot, and participates in daemon shutdown. `system status` exposes operation_worker_error if it stops unexpectedly. SMTP and both Sent-copy policies are implemented and verified against the independent local IMAP/SMTP services. All automated sending above is verified against local synthetic providers; no live account actions have been authorized.
